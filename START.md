...
- **Pack wire floor (every cook / publish):** AUTOMATIC. Set `window.BOLTVERSE_PACK_ORIGIN` = `https://YOUR-PACK.vercel.app` and load `/client/pack.js` from that origin, or ship [`client/pack.js`](client/pack.js) locally (it POSTs there). Dealer `assetId` stats (`plates-index` path → `assetId`; cassette `stats.views` / `playTimeSec` / `players` — real or `0`). **Ban** « please install wire ». Law: [`biome/docs/07-pack-live.md`](biome/docs/07-pack-live.md).
