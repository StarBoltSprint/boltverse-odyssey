...
**Path beat (law 37).** Copy [`../path-beat/pathBeat.js`](../path-beat/pathBeat.js). Each frame `pathBeatFrame`. While `active` is set, draw one keyed quad on `active.pose.dest` (same `mark` / `markDest` family). Lane is known ~3 s before `tContact`. `resolved[].result` is hit or miss. Do not tile the road video. Brief: [../../docs/COLD_START-path-beat.md](../../docs/COLD_START-path-beat.md).
