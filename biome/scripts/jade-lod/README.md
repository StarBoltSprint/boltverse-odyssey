...
**Height.** [`height.ts`](height.ts). Law [50](../../docs/50-heightfield-posture.md). Posture only. `h0 = 0.20 + 0.20 * fbm(x/28, z/28, s+101)` with 3 octaves. The valley (law [54](../../docs/54-path-noise-valley.md), [`path.ts`](path.ts)) flattens toward 0.20 with one `d`, shoulders `0.55`–`1.15` of `pathHalf`. Not `abs(x)`. Bolt snaps `h + 0.45` every tick. A kit stores `groundY` once. The cylinder base is that `groundY`, never 0. The sol film stays `y = 0`.
