...
**Particles.** Law [53](../../docs/53-gpu-particles.md). [`particles.ts`](particles.ts). One Points draw, 1400 slots. The CPU writes births. The vertex shader moves them. Mote jade 0.18 s, paw orange while sprinting past 3.2, Howl cyan 80. HUD `fx GPU`. Sparks are an overlay. They do not spawn trees. Snippets: [`particles.vert.glsl`](particles.vert.glsl), [`particles.frag.glsl`](particles.frag.glsl).
