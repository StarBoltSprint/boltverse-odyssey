...
**START HERE:** a playable biome composite (any paint) pastes [`docs/COLD_START-biome-method.md`](docs/COLD_START-biome-method.md) first. Obey the **FAIL / HARD** block at the top of that paste. The world is Imagine videos used as assets. No other language or engine builds the world. Three.js, Babylon, Unity, Unreal-as-world, Vite/TS, raw WebGL meshes, or a procedural shader scene with no Imagine mp4s is FAIL (Velum-class). The player is Bolt, the StarBoltSprint dog already on this GitHub. Use the sealed GPU Bolt (`lock/bolt-gallop-cycle.mp4`). Do not invent a player craft. A manta, ray, ship, or mesh hero is FAIL (Velum manta-class). Details (rocks, ruins, vapor, crystals, path dressing, terrain relief dressing, vegetation dressing beyond the sealed herbe/arbres cook) are also Imagine Video — cooked plates or keyed layers over densify/sol/ciel. Mesh or live procedural generators for path, terrain, vegetation, ruins, or details (old Three.js PathGenerator / Terrain / Detail class) are FAIL. Simplex placement of already-cooked herbe/arbres stays. Simplex (and placement noise) places already-cooked Imagine assets only. It does not draw the world. Drawing is Imagine Video. Noise that paints terrain, relief, or details is FAIL. GPU only composites keyed layers over those videos. No playable hang until the mp4s exist and play. That paste holds both world cooks: **A** road densify (one continuous Imagine plate, law 20) and **B** forest / open ground (law 43 — black, four skies, one ground tile, horizon, GPU trees and Bolt). Densify / QC order for a road stays [`scripts/biome-cook/README.md`](scripts/biome-cook/README.md) · [`docs/COLD_START-biome-cook.md`](docs/COLD_START-biome-cook.md). Imagine Live keyed props stay [`docs/COLD_START-imagine-engine.md`](docs/COLD_START-imagine-engine.md) (laws 39–42). Law 44 is the Imagine volume stack on open ground ([docs/44-imagine-volume-stack.md](docs/44-imagine-volume-stack.md)): collision capsule and look parallax on the existing spawn rows. Do not invent a second law 43 or a second volume law. Law 45 is the shader-graph look kitchen on those cards ([docs/45-shader-graph-look-kitchen.md](docs/45-shader-graph-look-kitchen.md)): sample, fade alpha, cutout. It does not spawn, pick LOD, or own the capsule. Do not invent a second look-graph law. Law 46 is the four picture jobs on that same seed (sheets, ground/sky plates, two-plane tree, height as posture only): [docs/46-four-picture-jobs.md](docs/46-four-picture-jobs.md). The film still has no thickness. Do not invent a second picture-job law. Law 47 is the Jade sheet cook (cutouts, not places): [docs/47-jade-sheet-cook.md](docs/47-jade-sheet-cook.md). One subject on empty air. Do not invent a second sheet-cook law. Law 48 is the Jade plate cook (empty ground and sky, the room not the furniture): [docs/48-jade-plate-cook.md](docs/48-jade-plate-cook.md). Do not invent a second plate-cook law. Law 49 is the two-plane tree kit (bole stays a mask, crown may dissolve, one fade slot per tree): [docs/49-two-plane-tree.md](docs/49-two-plane-tree.md). Do not invent a second two-plane law. Law 50 is height as posture only (`s+101`, L = 28, three octaves, the sol film stays flat): [docs/50-heightfield-posture.md](docs/50-heightfield-posture.md). Do not invent a second height law. Law 51 is the octave lock (height 3, ruin 2, page and L stay put): [docs/51-octave-map.md](docs/51-octave-map.md). Do not invent a second octave law. Law 52 splits engine from play (memory ring, bandDraw, path noise, Howl burst, sealed Bolt): [docs/52-engine-vs-play.md](docs/52-engine-vs-play.md). Do not invent a second hero. Law 53 is the GPU particle overlay (one Points draw, CPU births only, Imagine texture obligatoire): [docs/53-gpu-particles.md](docs/53-gpu-particles.md). Particles are not the world. A flat colored disc is FAIL. Law 54 is the path valley (page `s+7`, one distance for spawn, height, and tint): [docs/54-path-noise-valley.md](docs/54-path-noise-valley.md). Do not invent a second path. Law 55 is holdBand versus bandDraw (near 24 / mid 64 / far 96; the budget never writes prev): [docs/55-budget-banddraw.md](docs/55-budget-banddraw.md). Do not invent a second budget law.
