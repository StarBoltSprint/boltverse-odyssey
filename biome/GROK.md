...
**HARD — path beat (law 37, SmiR 2026-09-23).** Chemin reveals one lane ~3 s ahead of contact (`lookahead` 3.0) so the player can SIDES. Hit / miss at contact. Not under the paws. Not the whole densify plate. Same Howl cone as Lena. LOD fills the world; `pathBeatFrame` tells which lane to be in. Law: [docs/37-path-beat.md](docs/37-path-beat.md). Paste: [docs/COLD_START-path-beat.md](docs/COLD_START-path-beat.md). Runtime: [scripts/path-beat/pathBeat.js](scripts/path-beat/pathBeat.js) (`pathBeatFrame`).
