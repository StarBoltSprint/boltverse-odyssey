...
**PRIORITY 0 COMPOSITE GATE** — after REUSE, **BEFORE** Hang: **scale** (withers ~0.22–0.32 of frame; paws lower third; Bolt must **NOT** fill lane width; MUST `computeScale` / `assertScale` — [13d](13d-auto-scale.md) + [`biome/scripts/bolt-scale/`](../scripts/bolt-scale/)) + **light** (grade FROM this empty plate family) + **contact** (paw shadow multiply on the *road*). Order: key → despill ([13c](13c-green-despill.md)) → **`computeScale` / `assertScale`** ([13d](13d-auto-scale.md)) → plate grade → contact → shared grain. Law: [13](13-make-bolt-lane.md) · [13b](13b-anti-sticker-contact.md) · [13d](13d-auto-scale.md). **FAIL** if Grok keys the cycle and hangs without that proof.
...
- Bolt motion for a new biome = **REUSE** [`lock/bolt-gallop-cycle.mp4`](../../lock/bolt-gallop-cycle.mp4). **FAIL** if Grok invents a new sprint / recooks `bolt.mp4` from Imagine. Only SmiR can replace the lock. **BAN** copying hung `biome/master/bolt.mp4` / canyon Bolt as identity. Never delete the road library.
...
When a player asks **make a biome** / **add a biome** / cook a new sprint run: décor scrolls (rush); Bolt sprints **IN PLACE** (treadmill) via the **locked cycle**. Mid-lane. **48fps** on cooked plates. Style teacher = [`lock/bolt-back.jpg`](../../lock/bolt-back.jpg) **or** [`biome/lock/bolt-back.jpg`](../lock/bolt-back.jpg) (same bytes; `@ref`) if a still / repose is needed. **PRIORITY 0 — REUSE, not invent:** do **not** cook a new Bolt gallop. Black void on the style teacher = STYLE only. Hang ≠ wipe.
...
6. **PRIORITY 0 COMPOSITE GATE** — scale + plate light + paw contact **before Hang**. After key, MUST `computeScale` / `assertScale` ([13d](13d-auto-scale.md)). Proof (before/after stills or smoke). [13](13-make-bolt-lane.md) · [13b](13b-anti-sticker-contact.md) · [13d](13d-auto-scale.md). **FAIL** if Grok keys the cycle and hangs without that proof.
...
| **Pipeline** | Video A + **REUSE** sealed cycle → key + composite → **COMPOSITE GATE** scale+light+contact before Hang. Do not invent a sprint. [10-bolt-cutout-law.md](10-bolt-cutout-law.md) · [13](13-make-bolt-lane.md) · [13b](13b-anti-sticker-contact.md) · [13d](13d-auto-scale.md). |
...
## Étape 2 — Bolt (**REUSE** the sealed cycle — do not recook)

**HARD law:** [10-bolt-cutout-law.md](10-bolt-cutout-law.md). **PRIORITY 0 — REUSE, not invent.**

Take sealed [`lock/bolt-gallop-cycle.mp4`](../../lock/bolt-gallop-cycle.mp4) (already rear / green / rotary). Key + despill + composite onto Video A. Then **PRIORITY 0 COMPOSITE GATE**: scale + plate light + paw contact **before Hang**. **FAIL** if Grok invents a new Bolt sprint clip. **FAIL** if Grok keys the cycle and hangs without that proof. Only SmiR can authorize `imagineBoltClip` / a new cycle cook to replace the lock.

STYLE teacher = [`lock/bolt-back.jpg`](../../lock/bolt-back.jpg) **or** [`biome/lock/bolt-back.jpg`](../lock/bolt-back.jpg) if a still / repose is needed. Hung `biome/master/bolt.mp4` is OUTPUT only. **Not** hall `imagineClip`. **Not** chat Imagine.

Play = Vlahos + crown sat kill + feather 1 px + ombre ellipse aux pattes. [05-key.md](05-key.md). Then **PRIORITY 0 COMPOSITE GATE** — scale + plate light + paw contact on the *road* ([13](13-make-bolt-lane.md) · [13b](13b-anti-sticker-contact.md) · [13d](13d-auto-scale.md)). L/M/R = X shift of **one** Bolt layer.  
**Ne pas** redessiner le chien. **Ne pas** inventer un nouveau sprint. **Ne pas** cuire Bolt dans un seul film final.
...
2c. COMPOSITE GATE: computeScale/assertScale (13d) + plate light + paw contact proof BEFORE Hang (13 / 13b / 13c / 13d)
...
| Key sealed cycle and Hang without scale + plate light + paw contact | Giant dog / warm sticker on frost / float. PRIORITY 0 COMPOSITE GATE. **FAIL.** |
...
| `biome/docs/10-bolt-cutout-law.md` | HARD Bolt cutout — REUSE lock/bolt-gallop-cycle.mp4 → key + composite → COMPOSITE GATE 13/13b/13c/13d |
...
| `biome/docs/13d-auto-scale.md` | Auto-scale from the road — `computeScale` / `assertScale`. Grok must not pick size by eye. |
| `biome/scripts/bolt-scale/` | Cook tool — `node demo.js`; size from lane width. |
