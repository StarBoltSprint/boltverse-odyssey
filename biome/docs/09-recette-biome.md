...
| **Pipeline** | Video A + **REUSE** sealed cycle → key + composite → **COMPOSITE GATE** scale+gallop-clock+light+contact before Hang. Do not invent a sprint. [10-bolt-cutout-law.md](10-bolt-cutout-law.md) · [13](13-make-bolt-lane.md) · [13b](13b-anti-sticker-contact.md) · [13d](13d-auto-scale.md) · [14c](14c-gallop-clock.md). |
...
Take sealed [`lock/bolt-gallop-cycle.mp4`](../../lock/bolt-gallop-cycle.mp4) (already rear / green / rotary). Key + despill + composite onto Video A. Then **PRIORITY 0 COMPOSITE GATE**: scale + gallop-clock + plate light + paw contact **before Hang**. After REUSE + `bolt-scale`, MUST `gallop-clock` / `assertGallopClock` ([14c](14c-gallop-clock.md)). **FAIL** if Grok invents a new Bolt sprint clip. **FAIL** if Grok keys the cycle and hangs without that proof. **FAIL** if dogFps ≪ plateFps. Only SmiR can authorize `imagineBoltClip` / a new cycle cook to replace the lock.
...
Play = Vlahos + crown sat kill + feather 1 px + ombre ellipse aux pattes. [05-key.md](05-key.md). Then **PRIORITY 0 COMPOSITE GATE** — scale + gallop-clock + plate light + paw contact on the *road* ([13](13-make-bolt-lane.md) · [13b](13b-anti-sticker-contact.md) · [13d](13d-auto-scale.md) · [14c](14c-gallop-clock.md)). L/M/R = X shift of **one** Bolt layer.  
...
| Play sealed cycle as 1-of-N / stop-motion (~5–10fps dog, fluid road) | Saccadé + skate. After REUSE + `bolt-scale`, MUST `gallop-clock` / `assertGallopClock` ([14c](14c-gallop-clock.md)). **FAIL.** |
...
| `biome/docs/14c-gallop-clock.md` | Gallop clock anti-saccadé — native 96 fps, phase from `plate_time`, `assertGallopClock`. |
| `biome/scripts/gallop-clock/` | Cook/play tool — `node demo.js`; stop 1-of-N stepping. |
