...
A playable composite (lane, glance, wings, foes, howl, any paint) pastes `biome/docs/COLD_START-biome-method.md` first. This file is the **road** densify / QC order (section A of that paste). A forest or other open ground follows section B of that paste (black, four skies, one ground tile, horizon, GPU). Volume on that forest is law 44 (`44-imagine-volume-stack.md`). Card shading is law 45 (`45-shader-graph-look-kitchen.md`). Picture jobs are law 46 (`46-four-picture-jobs.md`). Sheet cook is law 47 (`47-jade-sheet-cook.md`). Plate cook is law 48 (`48-jade-plate-cook.md`). The two-plane tree is law 49 (`49-two-plane-tree.md`). Height as posture is law 50 (`50-heightfield-posture.md`). The octave lock is law 51 (`51-octave-map.md`). Engine versus play is law 52 (`52-engine-vs-play.md`). GPU particles are law 53 (`53-gpu-particles.md`, Imagine texture obligatoire). The path valley is law 54 (`54-path-noise-valley.md`). holdBand versus bandDraw is law 55 (`55-budget-banddraw.md`). None of these is this kit. Do not run this 3-lane kit as the forest world.
...
**Light layers + openables (law 38):** do not bake playable lights or open states into that plate. Imagine cooks light-only keyed plates and closed / open / transition bibs. GPU owns intensity, tint, hit, and open/close. Same Howl cone. Plate zones `road` | `sideL` | `sideR`: gameplay on the lanes; décor, lights, openables, and generators may also sit on the shoulders. Densify sides stay relatively empty. FAIL side clutter in Video A. Pack RT: densify and bibs look path-traced (soft GI, reflections, soft shadows) baked in the pixels. Interactive response is a gloss overlay graded from the plate, not a ray bounce. FAIL flat plastic lighting. FAIL real-time raytracing via Imagine. True RT is the UE rail, HOLD. Read `biome/docs/38-gpu-light-openable.md`. Paste `biome/docs/COLD_START-gpu-light-openable.md`. Runtime: `biome/scripts/gpu-light/gpuLight.js` (`lightLayerFrame`) · `biome/scripts/gpu-openable/gpuOpenable.js` (`openableFrame`).
