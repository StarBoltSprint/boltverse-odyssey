...
GPU compositor (no CPU `getImageData` hot path) · sealed **6 s** gallop · auto-scale · plate-matched grade · contact shadow · **biome-adaptive ground FX** (paw prints / splashes / dust) · hang ≠ wipe · Pack auto-wire.
...
| Compositor | **GPU from frame 0** — copy `biome/scripts/bolt-key-gl/` → Live `src/game/`. Ban CPU key hot path | [15](15-gpu-compositor.md) · [WIRE](../scripts/bolt-key-gl/WIRE.md) |
...
| Ground FX | **Biome table** below — paw trails + splash/dust, phase-locked to gallop | [16](16-biome-ground-fx.md) |
| Grade | From **this** empty plate family (hue/sat/rim) — never neutral studio | [13](13-make-bolt-lane.md) · [16](16-biome-ground-fx.md) |
...
3. **Grade / FX row** from [16-biome-ground-fx.md](16-biome-ground-fx.md) (cold/wet/ember/dust…).
...
4. REUSE lock cycle → key + despill (13c) — prefer GPU path even in cook QA.
5. `computeScale` / `assertScale` (13d).
6. Wire Live: `makeCompositor` **before** `getContext("2d")` ([15](15-gpu-compositor.md)).
7. `gallop-clock` / native loop 1× (14c).
8. Plate grade + contact shadow + **ground FX row for this biome** (16).
9. Hazards (spawn far, 1–2 lanes, plate-speed match).
10. Hang ≠ wipe + Pack wire + bump `VER`.
11. Smoke: dogFps ≈ min(96, display); no skate; shadow on road; FX visible on plant frames; no truck scale.

**FAIL** if: invent sprint · CPU key every rAF · wipe masters · hang without composite gate · skip FX row · bake Bolt into road mp4.
