...
9b3. **Law 37** path beat: copy `biome/scripts/path-beat/`. `pathBeatFrame` each tick. Target lane reveals ~3 s ahead (`lookahead` 3.0). SIDES resolves at contact (`hit` / `miss`). Same cone as Howl / Lena. Densify stays the loop. Do not tile it. Paste [COLD_START-path-beat.md](COLD_START-path-beat.md).
