...
**Sealed 2026-09-22 (SmiR).** Hang ≠ wipe. Recipe, not Live wiring. No new pixels in this law.
...
Attack plate: [`../prompts/howl-attack.txt`](../prompts/howl-attack.txt)  
...
## Howl — Imagine VIDEO only

File (when cooked): `howl-attack.mp4`. One plate for every biome. Not a shader.

Paste [`howl-attack.txt`](../prompts/howl-attack.txt) and do not rewrite the look:
...
- Progressive ~6s: tight at the **left** (Bolt’s mouth) then farther right. Energy leaves first and arrives later. The full tunnel does not pop in on frame 0.
...
GPU plays this plate as an FX quad from Bolt toward the current rail B target. It may scale and aim the quad. It may **not** redraw the rings.
...
2. Cook `howl-attack.mp4` from `howl-attack.txt` (Imagine VIDEO, black key, no shatter).
...
**FAIL if:** Howl is a shader · shatter is inside the Howl · black rectangles around shards · a meteor is marked destroyable · rail B is baked into the road · the obstacle is a sticker (no bounce, no contact, knife alpha) · one shatter plate serves every type.
