...
This is composite + states. Not raytracing. Same cone as Howl, Lena, and the path beat (`howlPose`). No second geometry stack.
...
6. **One film, one clock.** `tileDensify` and `coversPlate` are false. `clock` is `"densify"`. Do not slice Video A. Do not raytrace.
...
Draw each light with `draw === true` on `pose.dest` (intensity × tint). Draw each prop’s `bib` on `pose.dest`. When `content` is set and `content.visible`, draw that bib too. Grade from the plate. Do not write any of those pixels back into Video A.
