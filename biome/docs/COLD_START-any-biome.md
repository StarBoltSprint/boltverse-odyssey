...
2c. **`biome/docs/36-gpu-zones-lena-procedural.md`** ← **before** those stills and Video A. Densify stays one continuous plate (Rail A). GPU zones are keyed layers over it (Rail B). Do not chop one densify plate into spatial GPU tiles. Paste: `biome/docs/COLD_START-gpu-zones.md`. Runtime: `biome/scripts/lena-lod/lenaLod.js`. Bib: `biome/docs/COLD_START-lena-bib.md`.
2d. **`biome/docs/37-path-beat.md`** ← chemin reveals one lane ~3 s ahead of contact. SIDES, then hit/miss. Same Howl cone as Lena. Do not bake lanes into the densify loop. Paste: `biome/docs/COLD_START-path-beat.md`. Runtime: `biome/scripts/path-beat/pathBeat.js` (`pathBeatFrame`).
