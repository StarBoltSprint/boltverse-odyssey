...
2e. **`biome/docs/38-gpu-light-openable.md`** ← playable lights and openables are keyed GPU layers over densify. Imagine cooks light-only plates (beam / glow / neon / flash) and closed / open / transition bibs. GPU owns intensity, tint, on/off, cone, fade, hit, and open/close. Do not bake a beam or an open door into Video A. Pack RT: bibs and densify LOOK path-traced (soft GI, reflections, soft shadows) baked in the pixels. GPU gloss is a fake interactive overlay, not a ray bounce. FAIL flat plastic lighting. FAIL real-time raytracing via Imagine. True RT is the UE rail, HOLD. Paste: `biome/docs/COLD_START-gpu-light-openable.md`. Runtime: `biome/scripts/gpu-light/gpuLight.js` (`lightLayerFrame`) · `biome/scripts/gpu-openable/gpuOpenable.js` (`openableFrame`).
