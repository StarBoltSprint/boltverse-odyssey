...
2c′. **HARD OVERRIDE — `biome/docs/39-imagine-live-light.md`** ← playable lane props (crystal, fern, rock, globule, destroyable, dodge) are **Imagine Live first**. Imagine Video in the plate's three colors, pure black, clean luma key (`uClean`), no regrade. Same path as Bolt. Paste: `biome/docs/COLD_START-imagine-engine.md` · `biome/docs/COLD_START-imagine-live.md`. Runtime: `biome/scripts/imagine-live/imagineLive.js`. Law 36 still-bib LOD on a lane = FAIL. This step overrides the still-bib emphasis below.
2c. **`biome/docs/36-gpu-zones-lena-procedural.md`** ← architecture only, after 2c′. Densify stays one continuous plate (Rail A). GPU zones are keyed layers over it (Rail B). Do not chop one densify plate into spatial GPU tiles. Lane props that must look like Bolt use **law 39**, not still LOD. Paste: `biome/docs/COLD_START-gpu-zones.md`. Runtime: `biome/scripts/lena-lod/lenaLod.js`. Bib stays for architecture shoulders, not lane props: `biome/docs/COLD_START-lena-bib.md`.
2d. **`biome/docs/37-path-beat.md`** ← chemin reveals one lane ~3 s ahead of contact. SIDES, then hit/miss. Same Howl cone as Lena. Do not bake lanes into the densify loop. **FAIL** opaque rectangles and grey/beige shadow cards as obstacles or reveal UI. Reveal is native light/form, or HOLD until Imagine Live (law 39). Paste: `biome/docs/COLD_START-path-beat.md`. Runtime: `biome/scripts/path-beat/pathBeat.js` (`pathBeatFrame`).
2e. **`biome/docs/38-gpu-light-openable.md`** ← playable lights and openables are keyed GPU layers over densify. Imagine cooks light-only plates (beam / glow / neon / flash) and closed / open / transition bibs. GPU owns intensity, tint, on/off, cone, fade, hit, and open/close. Do not bake a beam, an open door, or side clutter into Video A. Plate zones are `road` | `sideL` | `sideR`: path beat, Howl hits, and critical openables stay on the lanes; décor LOD, lights, openables, and generators may also sit on the shoulders (`sideL` / `sideR`), same cone, `gradeFromPlate`, GPU over densify. Densify shoulders stay relatively empty. Pack RT: bibs and densify LOOK path-traced (soft GI, reflections, soft shadows) baked in the pixels. GPU gloss is a fake interactive overlay, not a ray bounce. FAIL flat plastic lighting. FAIL real-time raytracing via Imagine. True RT is the UE rail, HOLD. Paste: `biome/docs/COLD_START-gpu-light-openable.md`. Runtime: `biome/scripts/gpu-light/gpuLight.js` (`lightLayerFrame`) · `biome/scripts/gpu-openable/gpuOpenable.js` (`openableFrame`).
...
## Order
Teacher gate → **lane material (law 35)** → **GPU zones (law 36, layers not tiles)** → **empty still from law 20 defaults** → Video A (extract last frame → cook last still → session Imagine Video first+last, or CLI if `XAI_API_KEY` is set) → **law 23 geo qc PASS** → REUSE cycle → key/despill (luma protect) → scale → GPU wire (law 17) → clock → grade+bounce+shadow+FX row → hazards (law 25, dodge) → Howl targets only if the plan names them (law 32) → **law 33 seam PASS** vs previous plate → hang+Pack → smoke.
...
**FAIL** if: invent sprint · CPU key every rAF · copy scissor/IGN sketch · wipe masters · skip bounce · skip FX row · bake Bolt into road mp4 · chop densify into spatial GPU tiles (law 36) · bake the target lane into densify Video A (law 37) · bake a playable light or an open door / chest / generator / hatch into densify Video A (law 38) · SPH · ice-hole / Beat-narrow road as silent default · grow Bolt to fake withersMin on a wide road · raw plate mix into coat · 5-tap body smear · dark frost prints · hang a plate the geo qc FAILed · shader a Howl · bake shatter into the Howl · hang a seam the MAE qc FAILed · stop because `XAI_API_KEY` is unset · one-still I2V · “forcé localement” · claim MAE PASS without `plate-mae-qc.py`.

- still-LOD lane props (crystal / fern / rock as a graded still or sprite) = FAIL. Lane props = Imagine Live (law 39).
- path-beat opaque rectangles / grey-beige shadow cards as obstacles or reveal UI = FAIL.
