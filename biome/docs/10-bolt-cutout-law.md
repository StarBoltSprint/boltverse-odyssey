...
Read with [09-recette-biome.md](09-recette-biome.md) · [05-key.md](05-key.md) · [01-images.md](01-images.md) · [02-videos.md](02-videos.md) · [13-make-bolt-lane.md](13-make-bolt-lane.md) (Make Bolt Lane brief) · [13b-anti-sticker-contact.md](13b-anti-sticker-contact.md) (compositor anti-sticker + contact shadow) · [13c-green-despill.md](13c-green-despill.md) (green key + despill factory) · [13d-auto-scale.md](13d-auto-scale.md) (auto-scale from the road — `computeScale` / `assertScale`) · [14-rotary-gallop.md](14-rotary-gallop.md) (rotary cycle / plate-time cadence) · [14c-gallop-clock.md](14c-gallop-clock.md) (gallop-clock anti-saccadé — native 96 fps, `assertGallopClock`) · [15-gpu-compositor.md](15-gpu-compositor.md) (GPU compositor — hung [`bolt-key-gl.ts`](../scripts/bolt-key-gl/bolt-key-gl.ts); ban `getImageData` hot path; loop 1×) · [17-live-compositor.md](17-live-compositor.md) (Live compositor — plate bounce, luma protect, sin grain, FX quads. **FAIL** = scissor/IGN sketch).
...
**PRIORITY 0 COMPOSITE GATE** — after key, [13](13-make-bolt-lane.md) / [13b](13b-anti-sticker-contact.md) / [13d](13d-auto-scale.md) / [14c](14c-gallop-clock.md) / [15](15-gpu-compositor.md) is **mandatory** before KEEP / Hang: **SCALE** + **GALLOP CLOCK** + **GPU compositor** + plate **LIGHT** + paw **CONTACT**. After key, MUST run `computeScale` / `assertScale` ([`biome/scripts/bolt-scale/`](../scripts/bolt-scale/)) then **`gallop-clock` / `assertGallopClock`** ([`biome/scripts/gallop-clock/`](../scripts/gallop-clock/)) before KEEP. Native **96 fps** (no 1-of-N); `CYCLE_FRAMES=534`, `STRIDES_PER_CYCLE=22`; phase from `plate_time` (`strideHz≈4`); road `ds` from `dsPerFrame` (anti-skate). Play: loop 1×; no per-frame seek; no 534-canvas harvest. `assertGallopClock` **FAIL** if dogFps ≪ plateFps. Grok must not pick size by eye. Order: key → despill ([13c](13c-green-despill.md)) → **`computeScale` / `assertScale`** ([13d](13d-auto-scale.md)) → **`gallop-clock` / `assertGallopClock`** ([14c](14c-gallop-clock.md)) → GPU compositor ([15](15-gpu-compositor.md) / [17](17-live-compositor.md)) → plate grade + bounce → contact → **ground FX (16)** → shared grain. **FAIL** if Grok keys the cycle and hangs without scale + gallop-clock + light + contact proof (before/after stills or smoke).
...
**PRIORITY 0 — REUSE the sealed cycle.** [`lock/bolt-gallop-cycle.mp4`](../../lock/bolt-gallop-cycle.mp4) **is** the Bolt motion asset (**CANON ~5.56 s · 96 fps · 768×1168 · 534 frames**, rear / green / rotary). `CYCLE_FPS=96`, `CYCLE_FRAMES=534`, `STRIDES_PER_CYCLE=22`. Same bytes as `lock/bolt-gallop-cycle-96fps-loop6s.mp4`. Play: `loop=true`, rate **1×**. Never seek every frame. Never cache 534 canvases. Old `lock/bolt-gallop-cycle-0.93s-prev.mp4` (89-frame) = **ARCHIVE only — FAIL if used as play cycle**. Key + despill it. Composite onto Video A. Then **PRIORITY 0 COMPOSITE GATE** (13 / 13b / 13c / 13d / 14c / 15 / 16 / 17): scale + gallop-clock + GPU compositor + plate-grade + paw contact **BEFORE** KEEP. After key, MUST run `computeScale` / `assertScale` then `gallop-clock` / `assertGallopClock` (native 96 fps, no 1-of-N). Do **not** Imagine a new dog sprint. **FAIL** if Grok invents a new Bolt sprint clip. Only SmiR can authorize a new cycle cook to replace the lock. [`lock/bolt-gallop-cycle-first.jpg`](../../lock/bolt-gallop-cycle-first.jpg) = first frame of that 6 s / 96 fps cycle (repose / still only). [`lock/bolt-gallop-cycle-12s-preview.mp4`](../../lock/bolt-gallop-cycle-12s-preview.mp4) = preview of the archived short cycle only. Loop seam = **hard cut** — never optical-flow morph last→first.
...
| Clock | Gait = sealed [`lock/bolt-gallop-cycle.mp4`](../../lock/bolt-gallop-cycle.mp4) (**CANON ~5.56 s / 534 frames / 96 fps**, loop forever at 1×). Speed/scroll = plate. Do **not** cook a new sprint. Do **not** play the archived 0.93 s / 89-frame lock. |
...
6. **PRIORITY 0 COMPOSITE GATE** — after key, 13 / 13b / 13d / 14c / 15 / 16 / 17 is **mandatory** before KEEP / Hang: **SCALE** (withers ~0.22–0.32 of frame; paws lower third; Bolt must **NOT** fill lane width; MUST `computeScale` / `assertScale` — [13d](13d-auto-scale.md)) + **GALLOP CLOCK** (native 96 fps, no 1-of-N; phase from `plate_time`; `assertGallopClock` — [14c](14c-gallop-clock.md)) + plate **LIGHT** (grade FROM this empty plate family) + paw **CONTACT** (multiply on the *road*). Order: key → despill (13c) → **`computeScale` / `assertScale`** (13d) → **`gallop-clock` / `assertGallopClock`** (14c) → plate grade → contact → shared grain. **FAIL** if Grok keys the cycle and hangs without that proof.
...
**New biome Bolt = REUSE** [`lock/bolt-gallop-cycle.mp4`](../../lock/bolt-gallop-cycle.mp4). Key + despill ([13c](13c-green-despill.md)) → `computeScale` / `assertScale` ([13d](13d-auto-scale.md)) → **`gallop-clock` / `assertGallopClock`** ([14c](14c-gallop-clock.md)) → composite onto Video A → **COMPOSITE GATE** scale + gallop-clock + GPU + plate-grade/bounce + paw contact + FX ([13](13-make-bolt-lane.md) / [13b](13b-anti-sticker-contact.md) / [13d](13d-auto-scale.md) / [14c](14c-gallop-clock.md) / [15](15-gpu-compositor.md) / [16](16-biome-ground-fx.md) / [17](17-live-compositor.md)) **BEFORE** KEEP. Do **not** call `imagineBoltClip`.
