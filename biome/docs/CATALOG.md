...
| [52-engine-vs-play.md](52-engine-vs-play.md) | Engine draws. Play is the paws. geoRing 1 (3×3 mesh) versus memRing 2 (5×5 prev and groundY). bandDraw stays apart from holdBand. Far ↔ cull shrinks the impostor quad only (180 ms, scale 1 ↔ 0.35). Soft fern is a tuft disk (`r` 0.45), no push. Path noise, Howl burst, `lock/bolt.glb` as the Pack KEEP. No second wolf. Stubs in [`../scripts/jade-lod/`](../scripts/jade-lod/README.md). |
| [53-gpu-particles.md](53-gpu-particles.md) | GPU particle overlay. Playback tape, not a physics engine. One Points draw, 1400 slots. CPU writes births. Vertex shader integrates them. Every point samples an Imagine cutout (moss dust, ember, spark dust). Howl shatter's primary picture is `crystal_burst_vN`. A missing sheet hides the FX. A flat colored disc is FAIL. Rapier is not the weather. Stub [`../scripts/jade-lod/particles.ts`](../scripts/jade-lod/particles.ts). |
