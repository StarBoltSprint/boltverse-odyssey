...
Imagine Agent produced these two KEEP seals. Frozen **pose + décor** for this ice hall until SmiR reseals. Agent remains the primary cook path for future styles.
...
That writes `lock/SEAL-at-a.jpg` + `lock/SEAL-at-b.jpg` (720×1280) and replaces `lock/example-at-a.jpg` / `lock/example-at-b.jpg`.

`cook-room` **SKIPS** `imagineStill` for at-A and at-B when those SEAL files exist (`SEAL=1` / `SEAL_ATA=1` / `SEAL_ATB=1` also freeze). Hung PASS packs are not overwritten. **Smoke still gates.** Soft KEEP banned.
