...
Now the still itself: `creamHeight` → `gate.size punch-in` if h ≥ 0.55, `gate.size spawn-band` if spawn ∉ **0.22–0.32** (0.19 tiny FAILs). Sill FAIL if h < 0.28 or h > 0.45. **Size in band is not enough:** sit (dog-mask aspect < 2.2, loaf-turn, or sill cream w/h > 0.82) = `gate.sit`. Still yaw > 28° = `gate.yaw` (3/4 / profile). Dark muzzle in the upper-center = `identity.face` (back skull is bright). Sill dog top above 0.46 of frame = `gate.size punch-sill` (climbing the rift — recook the still, do not loosen). **Place:** `dogMask` `cx/W` — at-A ≤ **0.38** (teal LEFT), at-B ≥ **0.62** (gold RIGHT). Spawn / mid-hall `cx` **0.42–0.58** = `gate.place spawn-cx` even if h is 0.35–0.40. (Use the white-dog mask, not `creamPlace` — lock examples snap cream to a door blob.) **`gate.place` PASS is not a soft KEEP** — sit / yaw / face / size still FAIL (`lock/example-at-*-tiny` sit at the door and die). COOK smokes spawn **and** spawn↔sill Δh/H < 0.12 **before** films. A seated 0.37 at-B dies. A face-on 0.37 at-B dies. A 0.52 / top-0.32 climb dies. A 0.36 mid-hall dog dies. 0.19 dies at the still.
