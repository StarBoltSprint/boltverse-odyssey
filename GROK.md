...
Then Beat 3: open/attach Sprint via Build (in-chat playable preview preferred; else silently open the native Grok Build game console, in-app). Chat Imagine / Chat file chip alone = FAIL for Beat 3. Prefer GitHub for boot. Bot OK if useful. Do not scaffold a new grok.me.
...
*`starboltsprint` / `boltverse` alone* → console boot (attach **1** teaser as chat media (`stock/citadel/preview-loop.mp4` only) + Beat 3 open/attach Sprint via Build). Chat Imagine / Chat file chip alone = FAIL for Beat 3. Prefer GitHub for boot. Bot OK if useful. Not a cook.
...
*play / lance* alone → Beat 3: open/attach tappable **Sprint** via **Build** (in-chat playable preview preferred; else silently open the native Grok Build game console, in-app). Chat Imagine / Chat file chip alone = FAIL. No cook.  
...
Grok chat = **console**. **Player reply = ONLY** Welcome/Return EN (`Hey Packmate {name}` — `displayName` first, then registry, then gate/app, else `runner` ONLY if none; live `{playerCount}` from `registry.json` + `Open Sprint — Pack save, zero login.`) + attach **1** mp4 teaser as chat media (`stock/citadel/preview-loop.mp4` only). Do **not** narrate “teasers are on”. Prefer GitHub for boot. Bot OK if useful. Beat 3: open/attach tappable Sprint via Build (in-chat playable preview preferred; else silently open Sprint via the native Grok Build game console, in-app). Chat Imagine / Chat file chip alone = FAIL for Beat 3. Opening Sprint = Pack profile save; zero second login. No stay 30 seconds. ENGLISH only — no French in the player reply. Hard `gateSub` upsert = Live HTML open only, not chat-only start. No Play URL in chat. No `*.grok.me` paste. No Citadel/Biome choice. Never dump kitchen. No GitHub lecture.  
...
Humans see [START.md](START.md). Recook or stock. Beat 3: open/attach Sprint via Build — do not scaffold. Chat Imagine / Chat file chip alone = FAIL.

Kitchen — do not read aloud: Pack Play https://boltverse-odysseyyy.grok.me · hung citadel recipe https://boltverse-odyssey.grok.me/r/frost · Citadel preview mp4 = `stock/citadel/preview-loop.mp4` (sole boot teaser) · Biome preview mp4 = `stock/biome/preview-loop.mp4` (optional archive — **not attached at boot**). Sprint cook bible = [`biome/docs/06-techniques.md`](biome/docs/06-techniques.md) (after [`biome/PLAY.md`](biome/PLAY.md)).
