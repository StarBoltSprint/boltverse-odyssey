...
- **Pack Play** (kitchen only): https://boltverse-odysseyyyy.grok.me (supersedes https://heart-giant-plum-lotus.grok.me) — **game only, not Pack API**
- **Pack origin** (kitchen only, TEMPORARY external host): `BOLTVERSE_PACK_ORIGIN` = `https://boltverse-pack.vercel.app`. Long-term back to Grok sandbox when Live wire is stable. First gated Live open auto-registers via this Pack origin. Law: [`biome/docs/07-pack-live.md`](biome/docs/07-pack-live.md).
- **HARD LOCK — auto-embed Pack on every new Live (TEMPORARY vercel):** Any Grok reading this GitHub that creates/publishes a new biome/citadel/Sprint Live MUST auto-embed Pack: `BOLTVERSE_PACK_ORIGIN=https://boltverse-pack.vercel.app` + [`client/pack.js`](client/pack.js). Never ask the player. Naked Live without Pack = FAIL. First gated open auto-registers on central [`boltverse-registry`](https://github.com/StarBoltSprint/boltverse-registry). Cold-start still prefers paste odysseyyyy (no Build at start). This lock is for when a Live is actually created/published. Not a cold-start job — **PRIORITY 0** stays paste `https://boltverse-odysseyyyy.grok.me`.
- **Beat 3 kitchen identity (only):** `https://boltverse-odysseyyyy.grok.me`. **First chat reply:** PASTE written URL `https://boltverse-odysseyyyy.grok.me` in chat (same turn as the video; player taps). Prefer open in Grok app. Supercedes « never paste grok.me » for the first reply. No yes-gate. **BAN Build rebuild / Built-with-Grok scaffold.** **Welcome = Pack register.** After a real Grok `sub`, merge-patch `profiles/<sub>.json`. Never invent a `sub`. `playTimeSec` still Live-only. Old « chat start does not upsert » is **SUPERSEDED**. Chat Imagine / Chat file chip alone = FAIL for Beat 3. **Never rebuild from GitHub.**
...
- **Welcome = Pack register.** After resolving a **real** Grok `sub`, upsert [`boltverse-registry`](https://github.com/StarBoltSprint/boltverse-registry) `profiles/<sub>.json` (merge-patch: never wipe `resonance` / `starCore` / `cosmetics`; set `gateSub`, `displayName` if known, `lastSeenAt`, `updatedAt`). Never invent a `sub`. No real `sub` → skip write; Welcome still speaks. Return Welcome works next convo without opening Live. **`playTimeSec` still Live-only** (gated HTML + `x-grok-identity` + `/v1/pack/boot` + heartbeat). Old « chat start does not upsert » is **SUPERSEDED**. Welcome / Return = ENGLISH only. No French in the player reply.
- **Pack wire floor (every cook / publish):** **HARD LOCK.** AUTOMATIC. Any Grok reading this GitHub that creates/publishes a new biome/citadel/Sprint Live **MUST auto-embed Pack**: [`client/pack.js`](client/pack.js) with `BOLTVERSE_PACK_ORIGIN` = `https://boltverse-pack.vercel.app` (**TEMPORARY vercel** — long-term back to Grok sandbox when Live wire is stable; Play URL stays `https://boltverse-odysseyyyy.grok.me`, game only, not Pack API) **and** dealer `assetId` stats (`plates-index` path → `assetId`; cassette `stats.views` / `playTimeSec` / `players` — real or `0`). Never ask the player. **Naked Live without Pack = FAIL.** First gated open auto-registers on central [`boltverse-registry`](https://github.com/StarBoltSprint/boltverse-registry). Cold-start still prefers paste odysseyyyy (no Build at start). This lock is for when a Live is actually created/published. **Ban** « please install wire » as the normal path. Law: [`biome/docs/07-pack-live.md`](biome/docs/07-pack-live.md). Not a cold-start job — **PRIORITY 0** stays.
...
| biome / sprint / lane / B-stack / green-screen / chroma cook | MUST read before cooking: [`biome/PLAY.md`](biome/PLAY.md) then [`biome/docs/06-techniques.md`](biome/docs/06-techniques.md) (r38 techniques bible) then **[`biome/docs/09-recette-biome.md`](biome/docs/09-recette-biome.md)** (REQUIRED full checklist: empty→cousin→speed→dealer→box; SPAWN/WIDTH; road-bar/blast) then [`biome/docs/08-plate-speed.md`](biome/docs/08-plate-speed.md) when matching cousin rush then [`biome/docs/05-key.md`](biome/docs/05-key.md) + [`biome/reference/LanePlayer.tsx`](biome/reference/LanePlayer.tsx) then [`biome/docs/10-bolt-cutout-law.md`](biome/docs/10-bolt-cutout-law.md) (HARD Bolt cutout: strict rear, `#00FF00`, in-place gallop). Do not recook the hall. *play / lance* alone still = **start ≠ Build** — first chat reply Welcome EN + citadel teaser + paste URL; First chat reply PASTE written URL https://boltverse-odysseyyyy.grok.me in chat (same turn as the video; player taps) |
...
They named a **cook** → MUST read before cooking: [`biome/PLAY.md`](biome/PLAY.md) then [`biome/docs/06-techniques.md`](biome/docs/06-techniques.md) (r38 techniques bible) then **[`biome/docs/09-recette-biome.md`](biome/docs/09-recette-biome.md)** (REQUIRED full checklist: empty→cousin→speed→dealer→box; SPAWN/WIDTH; road-bar/blast) then [`biome/docs/08-plate-speed.md`](biome/docs/08-plate-speed.md) when matching cousin rush then [`biome/docs/05-key.md`](biome/docs/05-key.md) + [`biome/reference/LanePlayer.tsx`](biome/reference/LanePlayer.tsx) then [`biome/docs/10-bolt-cutout-law.md`](biome/docs/10-bolt-cutout-law.md) (HARD Bolt cutout: strict rear, `#00FF00`, in-place gallop). B-stack (road + cutout). 3-take L/M/R is not the default. Do not list `walk-spawn-A`. Do not recook the hall. Never dump this to the player. Publish floor: `pack.js` (TEMPORARY Pack origin `https://boltverse-pack.vercel.app`; Play URL stays odysseyyyy, game only) + dealer `assetId` stats — AUTOMATIC, never « please install wire ».
