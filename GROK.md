...
Acts: **breath** (loop, feet glued) | **walk** (one-shot, pose advances at `ended`).
...
Same door = no-op. During walk = ignore taps. Never walk→walk. `ended(walk)` always → `breath(arrive)` at t=0.

## Player (non-negotiable — these froze the hall before)

1. **Still underlayer always.** The still of the current pose sits under the videos. Never a black hole.
...
Join: `stillEnd(from) == stillStart(to)` → cut 0ms, else dissolve ≤ 280ms. Dissolve does not fix a bad encode.
