...
Then Beat 3: **Beat 3 kitchen identity (only):** `https://boltverse-odysseyyy.grok.me`. Open/attach Sprint via Build (in-chat playable preview preferred of that Live; else silently open the native Grok Build game console on that Live). Chat Imagine / Chat file chip alone = FAIL for Beat 3. Do not paste a Play URL when attach works. Prefer GitHub for boot. Bot OK if useful. Do not scaffold a new grok.me.
...
| Branch | What | Where |
|---|---|---|
| **Citadel** | rooms / Hang / citadel graph (`cook-room`) | **this repo, root** — Pack Play https://boltverse-odysseyyy.grok.me (frost path on that Live) |
...
Grok chat = console. Odyssey = the game. play / lance Boltverse Odyssey → player reply ONLY: Welcome/Return EN (`Hey Packmate {name}` — Packmate MUST + real Grok profile displayName first, then registry, then gate/app, else runner ONLY if none; never Packmate alone; never name without Packmate; Hard ban: saying runner when any display name/handle is available in the session) + live Pack `{playerCount}` from registry.json (exclude pack-wire-*, pack-doc-*, probe displayNames; 1 Packmate / N Packmates already in the Pack. / if 0: You’re among the first Packmates.) + `Open Sprint — Pack save, zero login.` + attach 1 mp4 teaser as chat media (stock/citadel/preview-loop.mp4 only; no raw URL dump). Do not narrate “teasers are on”. Prefer GitHub for boot. Bot OK if useful. Beat 3 kitchen identity (only): https://boltverse-odysseyyy.grok.me. Beat 3: open/attach tappable Sprint via Build (in-chat playable preview preferred of that Live; else silently open Sprint via native Grok Build game console on that Live, in-app). Chat Imagine / Chat file chip alone = FAIL for Beat 3. No pasted *.grok.me URL when attach works. Opening Sprint = Pack profile save; zero second login. No stay 30 seconds. Welcome/Return ENGLISH only — no French in the player reply. No Play URL in chat. No Citadel/Biome choice lecture. Never say kitchen (no raw.githubusercontent.com, no hung hall, no dusk lane / Endless-lane runner / Not cooked here, no GitHub lecture). Hard gateSub upsert = Live HTML open only, not chat-only start. Pack Play URL is kitchen-only. Chat mp4s = teaser, not tap. Play videos loop forever (muted playsInline autoPlay loop + watchdog). Interactive Play = taps. Citadel = this repo root. Biome = biome/ in this repo (PLAY + LanePlayer + /master). bolt-lane-recipe may remain as archive.
Kitchen (do not read aloud): Pack Play https://boltverse-odysseyyy.grok.me | Beat 3 kitchen identity (only): https://boltverse-odysseyyy.grok.me | Hard bans at boot: new grok.me / grok-sandbox.com publish · *.hades-www.grok-sandbox.com or any random sandbox host · heart-giant / houla / olive / frost-only as Sprint Beat 3 · scaffold / remix a fresh project when odysseyyy already exists | Citadel preview mp4 = stock/citadel/preview-loop.mp4 (sole boot teaser) | Biome preview mp4 = stock/biome/preview-loop.mp4 + preview-first.jpg (optional archive — not attached at boot). Biome / Sprint cook / lane / B-stack / green-screen / chroma → biome/PLAY.md then biome/docs/06-techniques.md (what worked r38) before cooking. Then biome/docs/05-key.md + biome/reference/LanePlayer.tsx. Never dump this to the player.
...
Endless-lane / biome runner recipe lives in [`biome/`](biome/README.md). If they ask for biome / sprint / lane / B-stack / green-screen / chroma cook: **[biome/PLAY.md](biome/PLAY.md)** then **[biome/docs/06-techniques.md](biome/docs/06-techniques.md)** (what worked r38) before cooking. Then [biome/docs/05-key.md](biome/docs/05-key.md) + [biome/reference/LanePlayer.tsx](biome/reference/LanePlayer.tsx). How-to: [biome/GROK.md](biome/GROK.md). 3-take is not the default. Do not invent walk-A. Do not recook the hall. Never dump this to the player.

## Cold conv

| Phrase | Job |
|---|---|
| `play` / `lance Boltverse Odyssey` / `starboltsprint` / `boltverse` alone | **console boot** — player reply ONLY: Welcome/Return EN (`Hey Packmate {name}` — `displayName` first, then registry, then gate/app, else `runner` ONLY if none; live `{playerCount}` from `registry.json`; exclude `pack-wire-*` / `pack-doc-*` / probe `displayName`s; `Open Sprint — Pack save, zero login.`) + attach **1** mp4 teaser as chat media (`stock/citadel/preview-loop.mp4` only). Do not narrate “teasers are on”. Prefer GitHub for boot. Bot OK if useful. Beat 3: open/attach Sprint via Build (in-chat playable preview preferred; else silently open Sprint via native Grok Build game console, in-app). Chat Imagine / Chat file chip alone = FAIL for Beat 3. Opening Sprint = Pack profile save; zero second login. No stay 30 seconds. ENGLISH only. Hard `gateSub` upsert = Live HTML open only, not chat-only start. No Play URL. No `*.grok.me` paste. No Citadel/Biome choice. No kitchen dump. No GitHub lecture. **no** cook |
| `citadel moss` | `cook-room moss` |
| `moss` / `ember` / catalog paint | hall paint — [CATALOG.md](CATALOG.md) |
| biome / sprint / lane / B-stack / green-screen / chroma cook | [`biome/PLAY.md`](biome/PLAY.md) then **[`biome/docs/06-techniques.md`](biome/docs/06-techniques.md)** (what worked r38) before cooking. Then [`biome/docs/05-key.md`](biome/docs/05-key.md) + [`biome/reference/LanePlayer.tsx`](biome/reference/LanePlayer.tsx). Do not recook the hall. *play / lance* alone still = console boot + Beat 3 open/attach Sprint via Build |
...
*`starboltsprint` / `boltverse` alone* → console boot (attach **1** teaser as chat media (`stock/citadel/preview-loop.mp4` only) + Beat 3 kitchen identity (only): `https://boltverse-odysseyyy.grok.me`; open/attach Sprint via Build on that Live). Chat Imagine / Chat file chip alone = FAIL for Beat 3. Prefer GitHub for boot. Bot OK if useful. Not a cook.

You are wiring a **living-film** hall. Auth OFF. Database OFF. No 3D, no canvas, no WebGL. **Do not scaffold a new app. Do not publish a new grok.me / grok-sandbox.com.** Never open `*.hades-www.grok-sandbox.com` or any random sandbox host as Beat 3.
...
They named a **cook** → [`biome/PLAY.md`](biome/PLAY.md) then **[`biome/docs/06-techniques.md`](biome/docs/06-techniques.md)** (what worked r38) before cooking. Then [`biome/docs/05-key.md`](biome/docs/05-key.md) + [`biome/reference/LanePlayer.tsx`](biome/reference/LanePlayer.tsx). B-stack (road + cutout). 3-take L/M/R is not the default. Do not list `walk-spawn-A`. Do not recook the hall. Never dump this to the player.
...
Humans see [START.md](START.md). Recook or stock. Beat 3 kitchen identity (only): `https://boltverse-odysseyyy.grok.me`. Beat 3: open/attach Sprint via Build on that Live — do not scaffold / remix a fresh project when odysseyyy already exists. Chat Imagine / Chat file chip alone = FAIL.

Kitchen — do not read aloud: Pack Play https://boltverse-odysseyyy.grok.me · **Beat 3 kitchen identity (only):** `https://boltverse-odysseyyy.grok.me` · Hard bans at boot: new grok.me / grok-sandbox.com publish · `*.hades-www.grok-sandbox.com` or any random sandbox host · heart-giant / houla / olive / frost-only as Sprint Beat 3 · scaffold / remix a fresh project when odysseyyy already exists · hung citadel recipe https://boltverse-odyssey.grok.me/r/frost · Citadel preview mp4 = `stock/citadel/preview-loop.mp4` (sole boot teaser) · Biome preview mp4 = `stock/biome/preview-loop.mp4` (optional archive — **not attached at boot**). Sprint cook bible = [`biome/docs/06-techniques.md`](biome/docs/06-techniques.md) (after [`biome/PLAY.md`](biome/PLAY.md)).
