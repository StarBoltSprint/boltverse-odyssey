...
Grok: packs/moss exists?
  ...
  no  → cook into packs/moss/ → validate-pack → same URL
```
...
FAIL → recook. Do not push. Do not give `/r/<id>`. See [VALIDATE.md](VALIDATE.md).
...
4. If they want a new WorldLine → cook pack into `packs/<id>/` (CHAR, COOK, encode HANG), run validate-pack, then the URL.
