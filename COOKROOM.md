...
4. atA / atB = `imagineStill` **edits that spawn** (bolt-back = coat; side ref = hung moss PASS sill still, else `lock/sill-at-*`) — skip each if hung PASS. Never `example-at-*`.
