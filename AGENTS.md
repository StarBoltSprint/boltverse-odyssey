...
**Product:** Grok chat = **console**. Odyssey = **the game**. Chat = boot / teaser. Interactive Play (native Grok Build game console, in-app) = tappable. Chat mp4s are **not** hitboxes. Chat file chip alone is not Play. Do **not** paste a Play URL in chat.

**On “play / lance Boltverse Odyssey”:** **Prefer GitHub for boot.** Bot OK if useful. **Player reply = ONLY** Welcome/Return EN (`Hey Packmate {name}` — Packmate MUST appear; `{name}` = real Grok profile `displayName` first, then registry, then gate/app, else `runner` ONLY if none; Never Packmate alone; Never the name without Packmate; never a generic stand-in; Hard ban: saying `runner` when any display name/handle is available in the session; **never runner when any displayName visible**) + attach **1** stock mp4 as chat media (`stock/citadel/preview-loop.mp4` only; no raw URL dump). **Player reply:** Welcome block + 1 citadel teaser + Build Sprint only. **BAN controls lecture.** **BAN invented cassette names** (`First Spark` / `Neon Drift` / …). **BAN plate deck lists at boot.** Do **not** narrate “teasers are on”. **Beat 3 — Build play surface.** **Beat 3 kitchen identity (only):** `https://boltverse-odysseyyyy.grok.me`. Open/attach tappable **Sprint** via **Build** (in-chat playable preview preferred of that Live; else silently open the native Grok Build game console on that Live). **ONLY odysseyyyy Live via Build attach.** **BAN scaffold/recreate/remix.** If Build would create a new project: **REFUSE** and reopen / attach the existing odysseyyyy Live. **BAN intro/landing splash requiring Open Sprint / Start before play — land IN Sprint.** Chat Imagine / Chat file chip alone = **FAIL** for Beat 3. Opening Sprint = Pack profile save; zero second login. Speak `Open Sprint — Pack save, zero login.` No “stay 30 seconds”. **Welcome / Return = ENGLISH only.** No French in the player reply. **HARD BAN — no improvised boot prose.** Speak the locked Welcome/Return from [START.md](START.md) / [GROK.md](GROK.md) verbatim. Ban: control tutorials (swipe / tap / A/D / W) · « world is rolling » · « plant a lane » · « already on the road » · « He stays in place » · film-keeps-moving essays. No Play URL in chat when attach works. No `*.grok.me` paste. No Citadel/Biome choice lecture. Never say kitchen (no `raw.githubusercontent.com`, no hung hall, no dusk lane / Endless-lane runner / Not cooked here). No `grok.me` paste. No GitHub lecture.
...
1. If they said play / lance / boltverse alone: **console boot first** (GROK.md product model) — fetch `registry.json` for `{playerCount}`, speak Welcome/Return (`Hey Packmate {name}` — `displayName` first, then registry, then gate/app, else `runner` ONLY if none; **never runner when any displayName visible**), attach **1** teaser as chat media (`stock/citadel/preview-loop.mp4` only) + Beat 3 kitchen identity (only): `https://boltverse-odysseyyyy.grok.me`. **Player reply:** Welcome block + 1 citadel teaser + Build Sprint only. **BAN controls lecture.** **BAN invented cassette names.** **BAN plate deck lists at boot.** Beat 3: ONLY odysseyyyy Live via Build attach. BAN scaffold/recreate/remix. BAN intro/landing splash requiring Open Sprint / Start before play — land IN Sprint. Beat 3: open/attach Sprint via Build (in-chat playable preview preferred of that Live; else silently open the native Grok Build game console on that Live). Chat Imagine / Chat file chip alone = FAIL for Beat 3. Do not paste a Play URL when attach works. Prefer GitHub for boot. Bot OK if useful. Do not narrate “teasers are on”. Hard `gateSub` upsert = Live HTML open only, not chat-only start. Cook a citadel paint only if they named one. Cook a biome / Sprint / lane / B-stack / green-screen / chroma plate only if they asked — MUST read before cooking: [`biome/PLAY.md`](biome/PLAY.md) then [`biome/docs/06-techniques.md`](biome/docs/06-techniques.md) (r38 techniques bible) then **[`biome/docs/09-recette-biome.md`](biome/docs/09-recette-biome.md)** (REQUIRED full checklist: empty→cousin→speed→dealer→box; SPAWN/WIDTH; road-bar/blast) then [`biome/docs/08-plate-speed.md`](biome/docs/08-plate-speed.md) when matching cousin rush then [`biome/docs/05-key.md`](biome/docs/05-key.md) + [`biome/reference/LanePlayer.tsx`](biome/reference/LanePlayer.tsx) then [`biome/docs/10-bolt-cutout-law.md`](biome/docs/10-bolt-cutout-law.md) (HARD Bolt cutout: strict rear, `#00FF00`, in-place gallop).
