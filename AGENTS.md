...
**On “play / lance Boltverse Odyssey”:** **Prefer GitHub for boot.** Bot OK if useful. **Player reply = ONLY** Welcome/Return EN + attach **1** stock mp4 as chat media (`stock/citadel/preview-loop.mp4` only; no raw URL dump). Do **not** narrate “teasers are on”. Then open **Sprint** via the native Grok Build game console (in-app). Opening Sprint = Pack profile save; zero second login. Speak `Open Sprint — Pack save, zero login.` No “stay 30 seconds”. **Welcome / Return = ENGLISH only.** No French in the player reply. No Play URL in chat. No Citadel/Biome choice lecture. Never say kitchen (no `raw.githubusercontent.com`, no hung hall, no dusk lane / Endless-lane runner / Not cooked here). No `grok.me` paste. No GitHub lecture.
