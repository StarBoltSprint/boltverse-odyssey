...
**Pack wire floor (every cook / publish):** AUTOMATIC. When Grok creates or publishes a biome, citadel, or Sprint Live, it **MUST** include [`client/pack.js`](client/pack.js) with `BOLTVERSE_PACK_ORIGIN` = `https://boltverse-pack.vercel.app` (**TEMPORARY** external host — long-term back to Grok sandbox when Live wire is stable; Play URL stays `https://boltverse-odysseyyyy.grok.me`, game only, not Pack API) **and** dealer `assetId` stats (`plates-index` path → `assetId`; cassette `stats.views` / `playTimeSec` / `players` — real or `0`). First gated Live open auto-registers via this Pack origin. Do not wait for the player to ask. **Ban** « please install wire » as the normal path. Law: [`biome/docs/07-pack-live.md`](biome/docs/07-pack-live.md). Not a cold-start job — **PRIORITY 0** first reply stays Welcome + teaser + paste. **Welcome = Pack register** stays.
