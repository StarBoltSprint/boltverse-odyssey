...
**HARD — Chat biome catalog (on ask).** If they ask **what biomes** / **playable biomes** / **list biomes** / **which runs** / **what can I play**: English Pack voice + short list of **hung** biomes only (from `plates-index` / hung `master` — never invent). Each line: name + optional short flavor. No kitchen paths. How to pick: `play canyon` / `play Tide` / `only Tide` — start that run or paste the Live + filter dealer. **On ask only — never on Welcome.** No chrome picker in the Live UI. Chat list + spoken pick is v1. Hang grows this catalog; wipe shrinks it = FAIL. Hung chapters **now**: canyon · cars · duel · night · war. Law: [`biome/docs/09-recette-biome.md`](biome/docs/09-recette-biome.md).
...
- Dump the chat biome catalog on Welcome / cold-start, or invent unhung biome names (catalog = on ask; hung chapters only)
