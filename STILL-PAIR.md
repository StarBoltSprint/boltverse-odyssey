...
Cook: feet on the **hall floor in front of** the rift, not inside the fill. at-A lock teacher = `lock/example-at-a.jpg` (SmiR hand still, standing BACK toward teal L, gold visible). Packs may copy it to `stills/at-a.jpg` when Smoke place is acceptable **OR** when owner seals (`SEAL_ATA=1` / `lock/SEAL-at-a.jpg`) — then **do not Imagine new at-A pose**. Unsealed: **copy PLACE+POSE from example; FORCE taille 0.35–0.40; FORCE STANDING; never shrink to 0.18; hall materials from spawn/catalog only — ignore example décor.** at-B teacher = hung moss PASS / swapped `lock/example-at-b` / `lock/sill-at-b`. Oval|RECT energy OK (oval preferred-ok). IGNORE a tiny ~0.18 crop the same way you IGNORE bolt-back ~0.53. Archived `lock/example-at-*-tiny` (sit + ~0.18) — **never send**. `gate.place` PASS on those tinies does **not** clear sit/yaw/size. atA/atB `image` = the spawn still. Do not relax these numbers to pass a climb.
